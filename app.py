...
try:
    ...
    col1.metric("歷史累積報酬率 (回測)", f"{total_return:.2f} %")
    ...
    with left_col:
        st.subheader("📈 淨值累積走勢 (過去一年)")
        fig_line = px.line(df_history, x='Date', y='portfolio_return', title="動態績效曲線")
    ...
    with col_search:
        search_ticker = st.text_input("輸入你想關注的股票代號 (如：2330.TW, NVDA)：", "")
    ...
    if search_ticker:
        try:
            ...
            if not hist.empty:
                ...
                st.success(f"💡 **買入試算：** 若以當前價格買入 **{simulate_shares:,} 股**，總共需要資金 **${total_cost:,.0f}**。")
                
            else:
                st.warning("抓不到報價，請確認代號是否正確（台股上市請加 .TW，上櫃請加 .TWO）。")
                
        except Exception as e:
            st.error("查詢失敗，請檢查股票代號格式。")
